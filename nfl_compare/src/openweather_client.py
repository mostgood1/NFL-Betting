...
def get_openweather_forecast(lat: float, lon: float, date_str: str, *, tz_name: Optional[str] = None,
    ...
    """
    Fetch a per-game weather snapshot for the given stadium coordinates and date using OpenWeather 5-day/3-hour forecast.
    - lat, lon: stadium coordinates
    - date_str: YYYY-MM-DD
    - tz_name: IANA timezone of the stadium (e.g., 'America/New_York'); if None, UTC is used
    - kickoff_hour_local: approximate local kickoff hour (24h)
    Returns dict with keys: 'wx_temp_f', 'wx_wind_mph', 'wx_precip_pct'
    """
    _load_env()
    ...
    try:
        ...
        # Choose target local datetime near kickoff
        ...
        if not chosen:
            # Fallback: take midday block of that date if present
